class Assets {
  static const String images =
      '/Users/myname/flutter_projects/petient_hub/assets/images/';
  static const String icons =
      '/Users/myname/flutter_projects/petient_hub/assets/icons/';
  static const String svg = 'assets/svg/';

  static const String mainSvg = '${svg}mainSvg.svg';
  static const String svglogo = '${svg}logo.svg';
  static const String iconPlus = '${svg}iconPlus.svg';
  static const String banner = '${svg}banner.svg';
  static const String playIcon = '${svg}playIcon.svg';

  static const String klinik = '${images}klinik.png';
  static const String homeOnFocus = '${icons}home_onfocus.png';
  static const String homeUnFocus = '${icons}home_unfocuse.png';
  static const String calendarOnFocus = '${icons}calendar_onfocus.png';
  static const String calendarUnFocus = '${icons}calendar_unfocus.png';
}

List<String> pngImages = [
  Assets.klinik,
  Assets.klinik,
  Assets.klinik,
];
